...
def main():
    creds = None
    if os.path.exists('/home/ubuntu/gmail-summarizer/token.json'):
        creds = Credentials.from_authorized_user_file('/home/ubuntu/gmail-summarizer/token.json', SCOPES)

    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            creds.refresh(Request())
        else:
            flow = InstalledAppFlow.from_client_secrets_file('/home/ubuntu/gmail-summarizer/credentials.json', SCOPES)
            creds = flow.run_local_server(port=0)
        with open('/home/ubuntu/gmail-summarizer/token.json', 'w') as token:
